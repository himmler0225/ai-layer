...
async def upsert_video(*, id: str, platform: str, title: str='', author: str='', views: int=0, likes: int=0, comments_count: int=0, url: str='', transcript: str='', metadata: dict | None=None) -> None:
    """Upsert video (async).
    ...
    factory = await get_session_factory()
    async with factory() as session:
        stmt = insert(Video).values(id=id, platform=platform, title=title, author=author, views=views, likes=likes, comments_count=comments_count, url=url, transcript=transcript, metadata_=metadata or {})
        excluded = stmt.excluded
        stmt = stmt.on_conflict_do_update(index_elements=[Video.id], set_={'title': excluded.title, 'author': excluded.author, 'views': excluded.views, 'likes': excluded.likes, 'comments_count': excluded.comments_count, 'url': excluded.url, 'transcript': func.coalesce(func.nullif(excluded.transcript, ''), Video.transcript), 'metadata': excluded.metadata, 'updated_at': func.now()})
...
async def update_transcript(video_id: str, transcript: str) -> None:
    ...
    factory = await get_session_factory()
    async with factory() as session:
        await session.execute(update(Video).where(Video.id == video_id).values(transcript=transcript, updated_at=func.now()))
        await session.commit()
