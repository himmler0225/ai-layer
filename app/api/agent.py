...
class AgentRequest(BaseModel):
    ...
    max_iter: Optional[int] = Field(
        None, ge=1, le=20, description="Defaults to AGENT_MAX_ITER from remote config"
    )
...
async def run(request: Request, body: AgentRequest):
    ...
    try:
        return ApiResponse.ok(
            await run_agent(body.task, tools, max_iter=max_iter, **kwargs)
        )
    ...
    async def generate():
        ...
        try:
            async for chunk in run_agent_stream(
                body.task, tools, max_iter=max_iter, **kwargs
            ):
                yield chunk
        except Exception as e:
            import json
