...
class AspectChunk(Base):
    ...
    __table_args__ = (
        ...
        Index('aspect_chunks_embedding_idx', 'embedding', postgresql_using='hnsw', postgresql_ops={'embedding': 'vector_cosine_ops'}),
    )
    id: Mapped[str] = mapped_column(Text, primary_key=True)
    movie_id: Mapped[str] = mapped_column(Text, ForeignKey('movies.id', ondelete='CASCADE'), nullable=False)
...
class AspectSummary(Base):
    ...
    __table_args__ = (
        ...
        Index('aspect_summaries_embedding_idx', 'embedding', postgresql_using='hnsw', postgresql_ops={'embedding': 'vector_cosine_ops'}),
    )
    id: Mapped[str] = mapped_column(Text, primary_key=True)
    movie_id: Mapped[str] = mapped_column(Text, ForeignKey('movies.id', ondelete='CASCADE'), nullable=False)
    aspect: Mapped[str] = mapped_column(Text, nullable=False)
