...
from app.services.agent.loop import begin_tool_round, bootstrap_agent, complete_tool_round, finish_agent, is_max_tokens_incomplete, resolve_final_text, run_tool_round, video_preview
...
from app.utils.openai_responses import extract_response_text, output_items_to_input, response_stream_with_retry, status_error
...
async def _emit_done(ctx: Dict, iteration: int, final_text: str) -> AsyncGenerator[str, None]:
    enriched = await finish_agent(ctx, iteration=iteration, final_text=final_text, include_summary=True)
