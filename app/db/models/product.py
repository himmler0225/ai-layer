...
"""ORM product RAG — raw / curated / aspect chunks / summaries."""
...
class Product(Base):
    """Sản phẩm / chủ đề phân tích review."""
...
class RawReview(Base):
    """L3 — review gốc crawl được."""

    __tablename__ = "raw_reviews"
    __table_args__ = (
        Index("raw_reviews_product_id_idx", "product_id"),
        Index("raw_reviews_product_likes_idx", "product_id", "likes"),
    )
...
class CuratedReview(Base):
    """Review đã lọc + xếp hạng theo likes."""
    ...
    raw_review_id: Mapped[str] = mapped_column(
        Text, ForeignKey("raw_reviews.id", ondelete="CASCADE"), nullable=False
    )
...
class AspectChunk(Base):
    """L2 — nhóm review theo aspect + vector."""

    __tablename__ = "aspect_chunks"
    __table_args__ = (
        Index("aspect_chunks_product_aspect_idx", "product_id", "aspect"),
        Index(
...
class AspectSummary(Base):
    """L1 — tổng hợp theo aspect + vector."""
