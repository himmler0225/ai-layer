...
async def get_ingest_queue_stats() -> dict:
    ...
    try:
        ...
        return {'enabled': True, 'ingest_enabled': settings.INGEST_ENABLED, 'exchange': settings.RABBITMQ_EXCHANGE or EXCHANGE_NAME, 'queues': queues, 'dlq': dlq, 'total_pending': total_pending, 'dlq_messages': dlq.get('messages'), 'management_ui_hint': 'http://localhost:15672 (user: ingest)'}
    finally:
        await conn.close()
