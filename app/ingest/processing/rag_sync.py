...
async def sync_comments_to_movie_rag(*, movie_hint: str, platform: str, video_id: str, raw_comments: list[dict]) -> str | None:
    ...
    if await _should_queue_summarize(movie_id, total):
        await publish(ROUTING_SUMMARIZE, platform=platform, video_id=movie_id, movie_hint=hint, payload={'movie_id': movie_id})
