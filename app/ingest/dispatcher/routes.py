...
async def route_tool(tool_name: str, inputs: dict, data: dict, *, product_hint: str, platform: str) -> None:
    ...
    if tool_name in ('youtube_get_detail', 'tiktok_video_info'):
        ...
        if mapped:
            await publish(ROUTING_VIDEO, platform=mapped['platform'], video_id=mapped['id'], product_hint=product_hint, payload={'video': mapped})
        return
    ...
    if tool_name == 'youtube_get_comments_batch':
        for entry in data.get('results') or []:
            if isinstance(entry, dict):
                await publish_comments(entry.get('video_id', ''), 'youtube', entry.get('comments') or [], product_hint)
        return
        ...
        if isinstance(results, dict):
            for video_id, entry in results.items():
                if entry and isinstance(entry, dict) and entry.get('text'):
                    await publish_transcript(str(video_id), 'youtube', entry['text'], product_hint, entry.get('language', ''))
        return
