...
from app.ingest.dispatcher.publish import SEARCH_TOOLS, publish_comments, publish_search, publish_transcript
...
async def route_tool(tool_name: str, inputs: dict, data: dict, *, movie_hint: str, platform: str) -> None:
    ...
    if tool_name in ('youtube_get_detail', 'tiktok_video_info'):
        ...
        if mapped:
            await publish(ROUTING_VIDEO, platform=mapped['platform'], video_id=mapped['id'], movie_hint=movie_hint, payload={'video': mapped})
        return
    if tool_name == 'youtube_get_comments':
        ...
        await publish_comments(video_id, 'youtube', comments, movie_hint)
        return
    if tool_name == 'youtube_get_comments_batch':
        for entry in data.get('results') or []:
            if isinstance(entry, dict):
                await publish_comments(entry.get('video_id', ''), 'youtube', entry.get('comments') or [], movie_hint)
        return
    if tool_name == 'youtube_get_transcript':
        ...
        if video_id and text and data.get('available', True):
            await publish_transcript(video_id, 'youtube', text, movie_hint, data.get('language', ''))
        return
        ...
        if isinstance(results, dict):
            for video_id, entry in results.items():
                if entry and isinstance(entry, dict) and entry.get('text'):
                    await publish_transcript(str(video_id), 'youtube', entry['text'], movie_hint, entry.get('language', ''))
        return
    if tool_name == 'tiktok_comments':
        ...
        if aweme_id and comments:
            await publish_comments(aweme_id, 'tiktok', comments, movie_hint, url=inputs.get('url', ''))
        return
    if tool_name == 'tiktok_transcript':
        ...
        if isinstance(data.get('segments'), list) and (not text):
            text = ' '.join((seg.get('text', '') if isinstance(seg, dict) else str(seg) for seg in data['segments'])).strip()
        if aweme_id and text:
            await publish_transcript(aweme_id, 'tiktok', text, movie_hint, data.get('language', ''))
