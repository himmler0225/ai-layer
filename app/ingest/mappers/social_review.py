...
def map_social_raw_review(movie_id: str, platform: str, video_id: str, raw: dict) -> dict | None:
    ...
    try:
        ...
        likes = 0
    return {'id': f'{platform}:{video_id}:{cid}', 'movie_id': movie_id, 'source': platform, 'source_video_id': video_id, 'author': raw.get('author') or '', 'content': content, 'rating': None, 'likes': likes, 'metadata': {'comment_id': cid}}
