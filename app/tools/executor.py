...
_SCHEMAS: Dict[str, Dict] = {
    ...
    for tool in (
        *(RAG_TOOLS if settings.RAG_ENABLED else []),
        *YOUTUBE_TOOLS,
        *TIKTOK_TOOLS,
        *UTIL_TOOLS,
    )
...
async def _youtube_get_by_topic(inp: Dict) -> Any:
    ...
    return await data_miner.get_by_topic(
        topic=inp["topic"], max_results=inp.get("max_results", 20)
    )
...
async def _youtube_get_live(inp: Dict) -> Any:
    ...
    return await data_miner.get_live(
        query=inp.get("query", ""), max_results=inp.get("max_results", 20)
    )
...
async def _youtube_get_comments_batch(inp: Dict) -> Any:
    ...
    return await data_miner.get_video_comments_batch(
        video_ids=ids[:8],
        max_per_video=inp.get("max_per_video", settings.AGENT_MAX_COMMENTS),
        sort=sort,
    )
...
async def _youtube_get_channel_videos(inp: Dict) -> Any:
    ...
    return await data_miner.get_channel_videos(
        channel_id=inp["channel_id"], max_results=inp.get("max_results", 30)
    )
...
async def _youtube_get_playlist_videos(inp: Dict) -> Any:
    ...
    return await data_miner.get_playlist_videos(
        playlist_id=inp["playlist_id"], max_results=inp.get("max_results", 30)
    )
