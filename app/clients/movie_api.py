...
from app.config.constants import HTTP_MAX_ATTEMPTS, HTTP_RETRY_STATUSES, MOVIE_API_TIMEOUT
from app.config.logger import Logger
from app.exceptions import AiLayerUpstreamError, AiLayerValidationError
...
def normalize_provider(name: str) -> MovieProvider:
    ...
    if key not in PROVIDERS:
        raise AiLayerValidationError(f"Unknown movie provider: {name!r} (use kkphim or ophim)")
...
async def _request(provider: str, method: str, path: str, *, params: Optional[Dict[str, Any]] = None) -> Any:
    ...
    raise AiLayerUpstreamError(
        f"{spec.name} request failed: {last_exc}",
        cause=last_exc,
    ) from last_exc
...
async def list_by_type(
    ...
    *,
    ...
    if movie_type not in MOVIE_LIST_TYPES:
        raise AiLayerValidationError(f"Invalid movie type: {movie_type!r}")
    spec = get_provider_spec(provider)
...
async def get_detail(provider: str, slug: str) -> Dict[str, Any]:
    ...
    if not slug:
        raise AiLayerValidationError("slug is required")
...
async def search(
    ...
    *,
    ...
    if not keyword:
        raise AiLayerValidationError("keyword is required")
    data = await _request(
        provider,
        "GET",
        "/v1/api/tim-kiem",
...
async def list_by_genre(
    ...
    *,
    ...
    slug = (slug or "").strip().strip("/")
    if not slug:
        raise AiLayerValidationError("genre slug is required")
    params = _list_query_params(
        page=page,
        limit=limit,
        category=category,
...
async def list_by_country(
    ...
    *,
    ...
    slug = (slug or "").strip().strip("/")
    if not slug:
        raise AiLayerValidationError("country slug is required")
    params = _list_query_params(
        page=page,
        limit=limit,
        category=category,
...
async def list_by_year(
    ...
    *,
    ...
    if year < 1900 or year > 2100:
        raise AiLayerValidationError("year must be a 4-digit year")
    params = _list_query_params(
        page=page,
        limit=limit,
        category=category,
...
async def proxy_webp(provider: str, *, image_url: str) -> Dict[str, Any]:
    ...
    if not spec.has_webp_proxy:
        raise AiLayerValidationError(f"{spec.name} does not support WebP image proxy")
    image_url = (image_url or "").strip()
    if not image_url:
        raise AiLayerValidationError("image_url is required")
