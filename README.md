...
`requirements.txt`/`requirements-dev.txt` are exact-pinned (`==`), not floors — this is what CI and Docker install, so it must stay reproducible. `requirements.lock.txt` is the full transitive closure (generated by installing both files into a clean venv and running `pip freeze`), kept for reference/debugging dependency resolution issues; it is not installed directly.

To upgrade a dependency: bump it deliberately in `requirements.txt`/`requirements-dev.txt`, reinstall in a clean venv, run `pytest` + `ruff check .`, regenerate `requirements.lock.txt`, then commit all three together.
